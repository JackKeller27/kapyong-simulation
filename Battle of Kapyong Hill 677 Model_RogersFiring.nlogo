...
; REPORTER: Compute point target hit probability from firing theory.

to-report point-hit-probability [lethal-radius sigma]
  ; Using: PK = 1 – exp( – (lethal-radius²)/(2*σ²) )
...
to setup
  ...
  ; Create 20 turtles for Team B
  create-turtles 700 [
    set team "B"
    set color blue
    set shape "person"
    setxy ((max-pxcor + min-pxcor) / 2 ) ((max-pycor + min-pxcor) / 2 )
    set hit? false
  ]
  reset-ticks
...
; GO
to go
   ; End simulation if one team is eliminated.
  if not any? turtles with [ team = "A" ] or not any? turtles with [ team = "B" ] [
    user-message "Combat is over."
    stop
  ]

  ; For each turtle (shooter), identify enemies in a cone in front.
  ask turtles [
    let shooter self
    ; Parameters for the adjacent path cone.
    let cone-angle 60            ; cone angle in degrees.
    let max-range 50             ; maximum engagement range.

    ; Find enemy turtles in the cone.
    ; We require that the enemy is within max-range and that the absolute difference
    ; between the shooter's heading and the direction (from shooter to enemy) is less than half the cone angle.
    let enemies-in-cone turtles with [
      team != [team] of shooter and
      distance shooter <= max-range and
      (abs ( subtract-headings ([heading] of shooter) (towards self) )) < (cone-angle / 2)
    ]

    if any? enemies-in-cone [
      if count enemies-in-cone > 50 [
        ; AREA FIRING: For many targets, each enemy in the cone is tested with area-hit-probability.
        ask enemies-in-cone [
          if (random-float 1 < area-hit-probability 10 20) [
            set hit? true
          ]
        ]
      ]
      if count enemies-in-cone < 50 [
        ...
        if enemy != nobody [
          if (random-float 1 < point-hit-probability 10 20) [
            ask enemy [ set hit? true ]
          ]
        ]
      ]
    ]
  ...
  ; Remove all turtles that were hit this tick.
  ask turtles with [ hit? = true ] [
    die
  ]

  ; Reset hit? flag for the next tick.
  ask turtles [ set hit? false ]
